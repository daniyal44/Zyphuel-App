...
* **Real-Time Dual Gmail Order Confirmation & Dispatch Emails (`RealtimeEmailEngine.kt`)**:
  ...
  * **Multi-Channel Real-Time Delivery Engine (`RealtimeEmailEngine.kt`)**: Backed by background Cloud Firestore `mail` trigger collections, public transactional REST mail relay via `OkHttpClient`, direct TLS/SSL SMTP socket communication, and Android system-level notification alerts for guaranteed inbox delivery.
