...
* **Real-Time Dual Gmail Order Confirmation & Dispatch Emails**:
  ...
  * **Multi-Channel Delivery Engine**: Backed by `dispatchRealtimeEmail` (internal audit logs, database notification persistence, and system-level heads-up notifications) and Android `ACTION_SEND` email intents (`message/rfc822`) for guaranteed real-time transmission.
