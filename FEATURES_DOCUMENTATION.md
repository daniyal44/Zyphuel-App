...
* **Real-Time Dual Gmail Order Confirmation & Dispatch Emails (`RealtimeEmailEngine.kt`)**:
  ...
  * **Multi-Channel Real-Time Delivery Engine (`RealtimeEmailEngine.kt`)**: Backed by **FormSubmit.co AJAX HTTPS Relay** (delivers real emails directly to inboxes without API keys), Cloud Firestore `mail` trigger collection, Formspree relay, direct TLS/SSL SMTP socket communication, and Android system-level notifications for guaranteed inbox arrival.
