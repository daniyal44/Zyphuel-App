...
* **Interactive Live Order Tracking Card (`RealTimeOrderTrackingCard`)**:
  ...
  * **Real-Time Dual Gmail Order Confirmation & Dispatch Emails**:
    * **Automated Customer Emails**: Upon placing an order (`placeOrder`), driver assignment (`acceptRiderOrder`), and each status progression (`changeOrderStatus`), the customer automatically receives real-time formatted emails to their registered Gmail. Details include Order ID, service item, quantity, COD amount, address, and live status.
    * **Automated Rider Emails**: Assigned riders receive immediate real-time emails to their registered Gmail address on new order dispatch assignments, order acceptance confirmations, and status completions.
    * **Multi-Channel Dispatch**: Backed by `dispatchRealtimeEmail` (internal audit logs) and Android `ACTION_SEND` email intents (`message/rfc822`) for guaranteed real-time transmission.
