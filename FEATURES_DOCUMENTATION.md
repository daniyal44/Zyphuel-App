...
* **Interactive Live Order Tracking Card (`RealTimeOrderTrackingCard`)**:
  ...
  * **Share Location Button (`share_location_btn`, `share_location_badge`)**: Replaces the previous "Edit Address" button. Tapping "Share Location" opens the native Android share sheet (`ACTION_SEND`) with the user's current GPS coordinates, resolved address text, and a Google Maps link (`https://maps.google.com/?q=lat,lng`). Users can share their delivery location via WhatsApp, SMS, Gmail, or any installed sharing app. Falls back with a toast if GPS is unavailable.
  * **Real-Time Gmail Order Confirmation Email**: After every successful order placement (`placeOrder`), the system dispatches a formatted order confirmation email to the customer's registered Gmail/email. The email includes Order ID, service type, quantity, total price, delivery address, and payment method. Uses both `dispatchRealtimeEmail()` (internal audit log) and Android `ACTION_SEND` intent with `message/rfc822` MIME type to trigger the device's email client (Gmail). Resolves the complaint that customers were not receiving email confirmations.
  ...
  * **Continuous Always-Visible Rider & Bowser Tracking on Google Maps SDK v6.4**: On both Google Maps SDK and high-fidelity Lahore vector maps, the delivery bowser/rider marker (🚚 / 🏍️) is always visible with dynamic heading rotation, vehicle plate badge, and radar halo. If an order is "Pending", the bowser is stationed at Green Town Depot; when "Delivering", it smoothly advances along the delivery corridor with live speed and ETA telemetry.
